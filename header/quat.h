...
inline Cvec4 quat2sk(Quat r) {
    double sin_s = sqrt(r[1] * r[1] + r[2] * r[2] + r[3] * r[3]);
    double cos_s = r[0];
    double s = atan2(sin_s, cos_s);
    Cvec3 k = Cvec3(r[1], r[2], r[3]);
    if (fabs(sin_s) > 1e-6) {
        k = k / sin_s;
    }
    return Cvec4(s, k[0], k[1], k[2]);
}

inline Quat sk2quat(Cvec4 sk) {
    double w = cos(sk[0]),
            x = sin(sk[0]) * sk[1],
            y = sin(sk[0]) * sk[2],
            z = sin(sk[0]) * sk[3];
...
inline Quat powerq(Quat q, double alpha) {
    if (q[0] < 0)q = q * (-1);
    Cvec4 sk = quat2sk(q);
    return sk2quat(sk * alpha);
}
